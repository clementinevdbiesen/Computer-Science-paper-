...
#Creates a binary matrix with a 1 if a certain feature is present in the description and is "Yes"
def yes_no_matrix(data):
    features_map_list = get_features_map(data)
    
    # Initiate the list of features with Yes and No values
    feature_names = len(extract_brand(data))
    # Creates a set to store the mapping of feature names to their respective index
    feature_index_map = {}
    
    # Adds features to feature name list if it has Yes or No values
    for features_map in features_map_list:
    ...
    # Calculates the elements of the matrix
...
#Creates a binary matrix with a 1 if a MW is present in a cleaned title string of a product
def binary_mat(MW, cleaned):
    input_matrix = np.zeros((len(MW), len(cleaned)), dtype=int)
    for i in range(len(cleaned)):
        for j in range(len(MW)):
